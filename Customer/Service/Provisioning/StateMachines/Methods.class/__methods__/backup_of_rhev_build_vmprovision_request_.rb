...
begin
  ...
  def get_network(build, matching_options_hash, matching_tags_hash)
    ...
    if matching_options_hash[:vlan].blank?
      ...
      matching_options_hash[:vlan] = 'external'
    end
  ...
  def set_valid_provisioning_args()
    ...
    valid_vmFields += [:linked_clone, :vm_prefix, :network_adapters, :placement_auto, :vm_description, :vm_auto_start, :placement_cluster_name, :customization_template_id]
