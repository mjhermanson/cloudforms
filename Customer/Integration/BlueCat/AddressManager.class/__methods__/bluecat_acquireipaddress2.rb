...
#
# An updated script originally written by ramrexx to support Savon 2.2.x
###################################
begin
  # Method for logging
  ...
  def call_BlueCat(prov)
    ...
    assignIP4Address = client.call(:assign_ip4_address) do |ctx|
      ctx.cookies auth_cookies
      ctx.message configuration_id: container_id, ip4_address: new_ipaddr, mac_address: '', host_info: prov.options[:vm_name], action: 'MAKE_STATIC', properties: ''
    end
    ...
    prov.set_option(:ip_addr, new_ipaddr.inspect )
